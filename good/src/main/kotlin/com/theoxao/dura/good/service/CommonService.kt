...
import com.theoxao.dura.good.common.CommonSearchView
...
class CommonService {
     ...
     * @param keyword: 搜索关键字
     */
    fun search(keyword: String, type: Int ,offset: Int): Any {
        return when (type) {
            0 -> {
                ...
                ).orderBy(query.field("id")?.desc()).limit(10).offset(offset)
            ...
            1 -> {
                return dslContext.select(TB_GOODS.ID, TB_GOODS.NAME, TB_GOODS.DESC, DSL.`val`(1).`as`("type"), TB_GOODS.IMAGES)
                    ...
                    .limit(10).offset(offset).fetchInto(CommonSearchView::class.java)
            }
            2 -> {
